...
=begin
The Array#chars method separtes a String object into an array of its individual
characters. The Array#each method is invoked on the name.chars array. The Array#each 
method always returns the array upon with the method is invoked. In this case,
the destructive String#upcase! method is invoked on each character, and the 
resultant array is returned. However this array was not assigned to a variable.
Thus `name` still references the original string argument and `shout_out_to(name)` 
will output `'Hey'` + the original `name` argument.
=end
...
=begin
On line 39, a ternary operator with the intention of comparing the method 
variable `odd_count` with the integer 3. Instead, `odd_count` is reassigned
to the integer three. This evaluates to `true` in the ternary expression.
=end
